...
py::array_t<FP> pyforward(omp_sampler* smplr, const py::list& theory, size_t number_of_events)
{
    tic("pybind");
    ...
    */
    ...
    //printf("C++ calc_grid_indices took: %zu us\n", us.count());
    //t0 = std::chrono::high_resolution_clock::now();
    //grid_idx = smplr->calc_grid_indices(py_weights.shape(1),py_weights.shape(2));
    //t1 = std::chrono::high_resolution_clock::now();
    //us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0);
    //printf("OMP calc_grid_indices took: %zu us\n", us.count());
    ...
    for (size_t i = 0; i < batch_size; i++){
      ...
      //us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0);
      //t0 = std::chrono::high_resolution_clock::now();
      //weight_mat = smplr->fast_calc_weight_matrix(n,max_n);
      //t1 = std::chrono::high_resolution_clock::now();
      //us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0);
      //printf("OMP calc_weight_matrix took: %zu us\n", us.count());
    ...
    //auto core_stop = std::chrono::high_resolution_clock::now();
    //auto core_time = std::chrono::duration_cast<std::chrono::seconds>(core_stop - core_start);
    //printf("OpenMP core took: %zu seconds.\n", core_time.count());
...
void sampler_delay_recording(omp_sampler* smplr){
  smplr->delay_recording();
}

void sampler_start_recording(omp_sampler* smplr){
  smplr->start_recording();
}

PYBIND11_MODULE(omp_wrapper, m) {
    m.doc() = "OpenMP Sampler";
    m.def("forward", &pyforward);
    pybind11::class_<omp_sampler>(m, "omp_sampler")
        .def(py::init())
