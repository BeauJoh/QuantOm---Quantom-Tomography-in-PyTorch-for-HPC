...
py::array_t<FP> pyforward(sycl_sampler* smplr, const py::list& theory, size_t number_of_events)
{
    ...
    */
    ...
    matrix<size_t> sycl_grid_idx = smplr->sycl_calc_grid_indices(py_weights.shape(1),py_weights.shape(2));
    t1 = std::chrono::high_resolution_clock::now();
    us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0);
    printf("SYCL version took: %zu us\n", us.count());
    std::unordered_set<size_t> diff;
    std::set_difference(grid_idx.begin(), grid_idx.end(), sycl_grid_idx.begin(), sycl_grid_idx.end(),
        std::inserter(diff, diff.begin()));
    //assert(diff.empty() && "Uh oh, there's a difference between SYCL and C++ versions of calc_grid_indices");
    */
...
void sampler_delay_recording(sycl_sampler* smplr){
  smplr->delay_recording();
}

void sampler_start_recording(sycl_sampler* smplr){
  smplr->start_recording();
}

void shutdown(sycl_sampler* smplr){
  smplr->shutdown();
}

PYBIND11_MODULE(sycl_wrapper, m) {
    m.doc() = "SYCL sampler";
    m.def("forward", &pyforward);
    pybind11::class_<sycl_sampler>(m, "sycl_sampler", py::module_local())
        .def(py::init())
